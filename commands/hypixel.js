...
module.exports = {
  ...
  exec : (call) => {
    if (call.args.length == 0) {
      call.message.channel.send("You need to provide a player name!")
          ...
          .then(player => {
            ...
            const statEmbed =
                new discord.MessageEmbed()
                    ...
                    .setThumbnail('https://minotar.net/avatar/' +
                                  player.nickname)
                    .setTimestamp()
    ...
    function getSkywarsStats(player, message) {
      hypixel.getPlayer(player)
          .then(player => { // requests api for the player object
            if (player.nickname.endsWith('s')) {
              var suffix = "'"
            } else {
              var suffix = "'s"
            ...
            const statEmbed =
                new discord
                    .MessageEmbed() // creates embed message with stats
                    .setColor('#ff007f')
      ...
      hypixel.getPlayer(player)
          .then(player => {
            ...
            const statEmbed =
                new discord.MessageEmbed()
                    .setColor('#ff007f')
                    .setTitle(player.nickname + suffix +
                    ...
                    .addFields(
                        {
                          name : 'Human',
                          value : `Kills: ${
                              player.stats.vampirez.human.kills} \n Deaths: ${
                              player.stats.vampirez.human
                                  .deaths} \n K/D Ratio: ${
                              player.stats.vampirez.human.KDRatio} \n Wins: ${
                              player.stats.vampirez.human.wins}`,
                          inline : true
                        },
                        {
                          name : 'Vampire',
                          value : `Kills: ${
                              player.stats.vampirez.vampire.kills} \n Deaths: ${
                              player.stats.vampirez.vampire
                                  .deaths} \n K/D Ratio: ${
                              player.stats.vampirez.vampire.KDRatio}`,
                          inline : true
                        },
                        {
                          name : 'Zombie',
                          value :
                              `Kills: ${player.stats.vampirez.vampire.kills}`,
                          inline : true
                        },
                    )
            message.channel.send(statEmbed);
    ...
    function getGuildStats(guildArg, message) {
      hypixel.getGuild('name', guildArg)
          .then(guild => {
