...
module.exports = {
  ...
  exec : async (call) => {
    ...
    if (!vc) {
      call.message.delete();
      return call.message.channel.send("Please join a voice chat first")
          .then(msg => {msg.delete({timeout : 10000})})
    } else {
      const playlistSearch = await searcher.getPlaylist(
          call.args.join(" ")) // Searches yt for the playlist
      const playlist =
          playlistSearch.sort(() => { // Shuffles the playlist into random order
            return 0.5 - Math.random();
          })

      sleep(
          6000) // Waits 6 seconds for searcher to finish inserting the playlist
      ...
      if (!serverQueue) {
        ...
        for (i = 0; i < playlist.length; i++) {
          queueConstructor.songs.push(
              playlist[i]); // push the playlist songs into queue
        }
    ...
    function play(guild, song) {
      ...
      console.log(`Now playing: ${
          song.title}`); // Announce what song is now playing to console
      const dispatcher =
      ...
      serverQueue.textChannel
          .send("I'm now playing: ```" +
                `${serverQueue.songs[0].title} || Duration: ${
                    serverQueue.songs[0].duration.minutes}:${
                    serverQueue.songs[0].duration.seconds}` +
                "```")
          .then(msg => {// announce to user what song is playing.
